...
void initData(PtpClock *ptpClock)
{
	...
	if ((CLOCK_IDENTITY_LENGTH == 8) && (PTP_UUID_LENGTH == 6))
	{
			...
			EUI48toEUI64(ptpClock->portUuidField, ptpClock->defaultDS.clockIdentity);  //clockIdentity = portUuidField
	}
...
/*when A,B's clockIdentity and portNumber are same return 1*/
