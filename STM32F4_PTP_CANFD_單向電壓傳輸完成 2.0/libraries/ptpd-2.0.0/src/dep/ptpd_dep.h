...
/**\{*/
#define getFlag(flagField, mask) (bool)(((flagField)  & (mask)) == (mask))    //determine flagField == mask; if yes return 1; no return 0;
#define setFlag(flagField, mask) (flagField) |= (mask)    //set the mask bit
#define clearFlag(flagField, mask) (flagField) &= ~(mask)    //clear the mask bit
...
bool  netMACInit(NetPath*, PtpClock*);
//bool  netShutdown(NetPath*);
bool  netMACShutdown(NetPath*);
int32_t netget(sys_mutex_t *, MACFrameTypeDef *);
