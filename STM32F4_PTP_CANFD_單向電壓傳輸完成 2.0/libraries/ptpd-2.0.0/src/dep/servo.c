...
void initClock(PtpClock *ptpClock)
{
	...
	ptpClock->ofm_filt.s = ptpClock->servo.sOffset;  //1
	...
	while(MACFrame_Array.tail != MACFrame_Array.head)  /*Empty MAC_frame*/
	{
		MACFrame_Array.tail = (MACFrame_Array.tail+1) & MACframe_QUEUE_MASK;      
		MACFrame_Array.MACFrame[MACFrame_Array.tail].length = 0;
	}
...
static void filter(int32_t * nsec_current, Filter * filt)
{
	...
	*/
	...
	/* Speedup filter, if not 2^s > n */  //filt->n = 0
	if ((1<<s) > filt->n)
	{
									...
									const TimeInternal *preciseOriginTimestamp, const TimeInternal *correctionField)
{
	...
	subTime(&ptpClock->Tms, syncEventIngressTimestamp, preciseOriginTimestamp);  //&ptpClock->Tms = syncEventIngressTimestamp - preciseOriginTimestamp
	subTime(&ptpClock->Tms, &ptpClock->Tms, correctionField);  //&ptpClock->Tms = &ptpClock->Tms - correctionField
	...
	/* Check results */
	if (abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) < DEFAULT_CALIBRATED_OFFSET_NS)  //abs:get positive value
	{
	...
	else if (abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) > DEFAULT_UNCALIBRATED_OFFSET_NS)  //abs:get positive value
	{
		if (ptpClock->portDS.portState == PTP_SLAVE)
		{
				setFlag(ptpClock->events, SYNCHRONIZATION_FAULT);
...
void updateClock(PtpClock *ptpClock)
{
	...
	if (ptpClock->currentDS.offsetFromMaster.seconds != 0 || abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) > MAX_ADJ_OFFSET_NS)
	{
		...
		if (!ptpClock->servo.noAdjust)  //noAdjust = 0  //Adjust
		{
			if (!ptpClock->servo.noResetClock)  //noResetClock = 0  //ResetClock
			{
				getTime(&timeTmp);
...
//	printf("updateClock: offset from master: %d sec %d nsec\n",
...
//			ptpClock->currentDS.offsetFromMaster.nanoseconds);
	...
	OffsetSecond = ptpClock->currentDS.offsetFromMaster.seconds;
	OffsetNanosecond = ptpClock->currentDS.offsetFromMaster.nanoseconds;
}
